...
// trigger update
f.$apply(function(){})},p=i;f.edit=function(){if(!f.aceEditor)return void(console&&console.error("'ace lib is missing'"));f.editActivated?(j&&(document.getElementById(f.id).env=null),n(p),i=p,f.parsable=!1):(p=i,j=ace.edit(f.id),j.setAutoScrollEditorIntoView(!0),j.setOptions({maxLines:1/0}),j.on("change",o)),f.editActivated=!f.editActivated},f.update=function(){f[m]=i,f.$emit("json-updated",i),f.onEdit&&f.onEdit({newJson:i}),this.edit()}}}}]).factory("ngPrettyJsonFunctions",function(){
